...
class OCREngine:
    def __init__(self, lang_list=["en"]):
        ...
        if _have_easyocr:
            try:
                import ssl
                # Bypass SSL verification to allow EasyOCR to download models on Windows
                try:
...
def llm_convert_to_latex(ocr_text: str, image=None, model="gpt-4o-mini") -> str:
    ...
    if gemini_key:
        try:
            prompt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "prompts", "latex_prompt.txt")
